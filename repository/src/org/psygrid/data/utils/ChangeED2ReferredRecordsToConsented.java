 ...
 *
 */
public class ChangeED2ReferredRecordsToConsented {
	...
	public static void main(String[] args) {
        ...
        try{
            ...
            for ( DataSetDTO dtods: datasets ){
            	if ( dtods.getProjectCode().equals("ED2")){
            		ds = rdao.getDataSet(dtods.getId()).toHibernate();
            		break;
            	}
            }
            
            if ( null == ds || !ds.getProjectCode().equals("ED2") ){
            	throw new RuntimeException("Couldn't find ED2 dataset");
            }
            ...
            List<Long> records = dao.getRecordsInStatusReferredByProject("ED2");
            for ( Long id: records ){
            	System.out.println("Changing status of record "+id);
            	rdao.changeStatus(id, statusId, "CN=Edie CPM, OU=users, O=psygrid, C=uk");
            }
