 ...
 *
 */
public class ChangeNEDReferredRecordsToConsented {
	...
	public static void main(String[] args) {
        ...
        try{
            ...
            for ( DataSetDTO dtods: datasets ){
            	if ( dtods.getProjectCode().equals("NED")){
            		ds = rdao.getDataSet(dtods.getId()).toHibernate();
            		break;
            	}
            }
            
            if ( null == ds || !ds.getProjectCode().equals("NED") ){
            	throw new RuntimeException("Couldn't find NED dataset");
            }
            ...
            List<Long> records = dao.getRecordsInStatusReferredByProject("NED");
            for ( Long id: records ){
            	System.out.println("Changing status of record "+id);
            	rdao.changeStatus(id, statusId, "CN=NEDEN CPM, OU=users, O=psygrid, C=uk");
            }
