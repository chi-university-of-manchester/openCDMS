...
public class OpcritFormTest extends DAOTest {

	private RepositoryDAO dao = null;	
	private Factory factory = null;

	private static final long dataSetId = 5;
	...
	public void testAddOpcrit() {
		//String name = "testAddOpcrit - "+(new Date()).toString();

		long dsId = dataSetId;

		try {
			DataSet dataSet = dao.getDataSet(dsId).toHibernate();

			ConsentFormGroup cfg = dataSet.getAllConsentFormGroup(0);
			DocumentGroup baselineA = dataSet.getDocumentGroup(0);

			Document opcrit = createDocument(dataSet);
			
			opcrit.addConsentFormGroup(cfg);
			DocumentOccurrence opcritOcc = factory.createDocumentOccurrence("Baseline");
			opcritOcc.setDisplayText("Baseline");
			opcrit.addOccurrence(opcritOcc);
			opcritOcc.setDocumentGroup(baselineA);
    ...
    protected static void createDocumentStatuses(
            Factory factory,
            Document document){
  ...
  */
        ...
        String WS_URL = "http://localhost:8080/"; //"https://localhost/";
        ...
        exDE.setExternalTransformer(transformer);
        doc.addEntry(exDE);
        ...
        for (int i = 0; i < doc.numEntries(); i++) {
        	...
        	if (entry instanceof OptionEntry || entry instanceof TextEntry) {
        		System.out.println("Entry added is "+entry.getLabel());
