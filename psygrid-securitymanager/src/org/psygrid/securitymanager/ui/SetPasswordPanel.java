...
public class SetPasswordPanel extends JPanel implements WizardPanel {
	...
	private JLabel passwordStrength = new JLabel();

	private JLabel mobileNumber;
	private JTextField mobileField = new TextFieldWithStatus(30, false);
	
	private String currentUserName;
	
	public SetPasswordPanel() {
		add(createMainPanel());//		JPanel mainPanel = new JPanel();
	}
	...
	private JPanel createFormPanel() {
		...
		mobileNumber = new JLabel(PropertiesHelper.getPropertyHelper().getStringFor(STRINGS_PREFIX +"entermobilenumber"), JLabel.TRAILING);
		mobileField = new TextFieldWithStatus(30, false);
		...
		generatePwdButton.addActionListener(
				new ActionListener(){
					public void actionPerformed(ActionEvent e) {
						...
						JOptionPane.showMessageDialog(SetPasswordPanel.this, "<html>Password is <b>"+password+"</b></html>", "Password", JOptionPane.INFORMATION_MESSAGE);
					}
				}
		);
		
		final JLabel reenterPasswordLabel = new JLabel(PropertiesHelper.getPropertyHelper().getStringFor(STRINGS_PREFIX+"reenterpassword"), JLabel.TRAILING);
	...
	private JPanel createHeaderPanel(String headerText) {
		...
		if (headerText == null) {
			headerPanel.add(new CustomLabel(PropertiesHelper.getPropertyHelper().getStringFor(STRINGS_PREFIX + "setpassword") + " " + UserModel.getInstance().getFirstname() + " " + UserModel.getInstance().getLastname()));
