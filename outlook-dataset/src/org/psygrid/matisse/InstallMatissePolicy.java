...
/**
 * Policy for Matisse (cloned from outlook etc.)
 *
 * @author pwhelan
 */
public class InstallMatissePolicy {

	public static void main(String[] args){


		GroupType[] groups = MatisseGroups.allGroups();

		PolicyDescriptionType pdt = new PolicyDescriptionType();
		pdt.setActions(RBACAction.allActions());
		TargetType[] tta = new TargetType[MatisseGroups.allAsTargets().length+RBACTarget.allAsTargets().length];
		for (int i = 0; i < MatisseGroups.allAsTargets().length; i++) {
			tta[i] = MatisseGroups.allAsTargets()[i];
		}
		int j=0;
		for (int i = MatisseGroups.allAsTargets().length; i < MatisseGroups
				.allAsTargets().length
		...
		PrivilegeType[] prta = new PrivilegeType[MatisseGroups.allAsPrivileges().length
				+ RBACRole.allAsPrivileges().length];
		for (int i = 0; i < MatisseGroups.allAsPrivileges().length; i++) {
			prta[i] = MatisseGroups.allAsPrivileges()[i];
		}
		int k=0;
		for (int i = MatisseGroups.allAsPrivileges().length; i < MatisseGroups
		...
		try {
			...
			try {
				...
				PolicyType pt = new PolicyType("Matisse", "MTS", pdt, sta);
