...
public class Reports {
	...
	public static IReport recruitmentInLondonReport(DataSet ds) throws ReportException {
		...
		IReport report = recruitmentReport(ds, groups, "Central and North West London Hub");
	...
	public static IReport ukCRNReport(DataSet ds) throws ReportException {
		...
		if (curDate.get(Calendar.MONTH) < Calendar.APRIL) {
			...
			endDate   = new GregorianCalendar(curDate.get(Calendar.YEAR), Calendar.MARCH, 0);
		}
		else {
			startDate = new GregorianCalendar(curDate.get(Calendar.YEAR), Calendar.APRIL, 0);
			endDate   = new GregorianCalendar(curDate.get(Calendar.YEAR) +1, Calendar.MARCH, 0);
		}
		chart.setTimePeriod(startDate, endDate);
