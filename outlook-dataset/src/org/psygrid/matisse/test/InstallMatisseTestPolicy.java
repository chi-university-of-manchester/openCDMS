 ...
 * @author pwhelan
 */
public class InstallMatisseTestPolicy {

	public static void main(String[] args){


		GroupType[] groups = MTSTGroups.allGroups();

		PolicyDescriptionType pdt = new PolicyDescriptionType();
		pdt.setActions(RBACAction.allActions());
		TargetType[] tta = new TargetType[MTSTGroups.allAsTargets().length+RBACTarget.allAsTargets().length];
		for (int i = 0; i < MTSTGroups.allAsTargets().length; i++) {
			tta[i] = MTSTGroups.allAsTargets()[i];
		}
		int j=0;
		for (int i = MTSTGroups.allAsTargets().length; i < MTSTGroups
				.allAsTargets().length
		...
		PrivilegeType[] prta = new PrivilegeType[MTSTGroups.allAsPrivileges().length
				+ RBACRole.allAsPrivileges().length];
		for (int i = 0; i < MTSTGroups.allAsPrivileges().length; i++) {
			prta[i] = MTSTGroups.allAsPrivileges()[i];
		}
		int k=0;
		for (int i = MTSTGroups.allAsPrivileges().length; i < MTSTGroups
		...
		try {
			...
			try {
				...
				PolicyType pt = new PolicyType("Matisse Test", "MTST", pdt, sta);
