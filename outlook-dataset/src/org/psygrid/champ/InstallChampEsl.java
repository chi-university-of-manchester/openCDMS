...
public class InstallChampEsl {
	...
	private static SAMLAssertion login(String[] args) throws Exception {
		...
		SAMLAssertion sa = qc.getSAMLAssertion(new ProjectType("CHAMP", "CHP", "CHAMP", "CHP", false));

		return sa;
	}

	private static IProject createProject() {
		IProject project = factory.createProject("CHP");
		project.setProjectCode("CHP");
		project.setProjectName("CHAMP");
		return project;
	}

	private static IProject createGroups(IProject project) {

		project.setGroup(factory.createGroup("North London",  "001001"));
		project.setGroup(factory.createGroup("East Midlands",  "001002"));
	...
	private static IProject setupRandomisation(IProject project) throws Exception {
		...
		if ( r == null) {
			r = factory.createRandomisation("CHP");
		}

        IStrata strata1 = factory.createStrata("centreNumber");
        strata1.setValue("001001");
        strata1.setValue("001002");
        r.getStrata().add(strata1);
        IStrata strata2 = factory.createStrata("Treatment Stream");
        strata2.setValue("Endocrinology");
        strata2.setValue("Cardiology");
        strata2.setValue("Respiratory");
        strata2.setValue("Medicine & Gastroenterology");
        r.getStrata().add(strata2);
        r.getTreatments().put("1", "CBT");
        r.getTreatments().put("2", "Control");
		...
		r9.setNotifyOfRSTreatment(false);
		roles.add(r9);
