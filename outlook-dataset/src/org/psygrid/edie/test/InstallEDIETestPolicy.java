 ...
 *
 */
public class InstallEDIETestPolicy {

	public static void main(String[] args){

		GroupType[] groups = EDTGroups.allGroups();

		PolicyDescriptionType pdt = new PolicyDescriptionType();
		pdt.setActions(RBACAction.allActions());
		TargetType[] tta = new TargetType[EDTGroups.allAsTargets().length+RBACTarget.allAsTargets().length];
		for (int i = 0; i < EDTGroups.allAsTargets().length; i++) {
			tta[i] = EDTGroups.allAsTargets()[i];
		}
		int j=0;
		for (int i = EDTGroups.allAsTargets().length; i < EDTGroups
				.allAsTargets().length
		...
		PrivilegeType[] prta = new PrivilegeType[EDTGroups.allAsPrivileges().length
				+ RBACRole.allAsPrivileges().length];
		for (int i = 0; i < EDTGroups.allAsPrivileges().length; i++) {
			prta[i] = EDTGroups.allAsPrivileges()[i];
		}
		int k=0;
		for (int i = EDTGroups.allAsPrivileges().length; i < EDTGroups
		...
		try {
			...
			try {
				...
				PolicyType pt = new PolicyType("EDIE Test", "EDT", pdt,  sta);
