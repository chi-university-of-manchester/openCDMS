 ...
 *
 */
public class InstallEDIETestEsl {
	...
	private static SAMLAssertion login(String[] args) throws Exception {
		...
		SAMLAssertion sa = qc.getSAMLAssertion(new ProjectType("EDIE Test", "EDT", "EDIE-2", "2171", false));

		return sa;
	}

	private static IProject createProject() {
		IProject project = factory.createProject("EDT");
		project.setProjectCode("EDT");
		project.setProjectName("EDIE Test");
		return project;
	}
	...
	private static IProject setupRandomisation(IProject project) throws Exception {
		...
		if ( r == null) {
			r = factory.createRandomisation("EDT");
		}
        ...
        r.getTreatments().put("EDT-000", "EDIE Test Control");
        r.getTreatments().put("EDT-001", "EDIE Test CognitiveBehaviourTherapy");
