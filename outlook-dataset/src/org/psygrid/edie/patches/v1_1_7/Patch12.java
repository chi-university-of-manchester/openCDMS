...
public class Patch12 extends AbstractPatch {
	...
	public String getName() {
		return "Fix SCID at Study Termination/Transition to allow multiple diagnoses";
	}
	...
	public void applyPatch(DataSet ds, String saml) throws Exception {
		...
		Document scid = ds.getDocument(14);
	...
	public Object preApplyPatch(DataSet ds, RepositoryClient client, String saml) throws Exception {
		...
		//Map of record id -> map of doc occ id -> list of SCID data (zeroth element is oldest value, nth is newest value)
		...
		Map<Long, Map<Long, List<Object[]>>> data = new HashMap<Long, Map<Long, List<Object[]>>>();
		...
		DocumentOccurrence transitionOcc = scid.getOccurrence(0);
		if ( !"Transition".equals(transitionOcc.getName()) ){
			throw new RuntimeException("This is not the Transition occurrence, it is "+transitionOcc.getName());
		}

		DocumentOccurrence terminationOcc = scid.getOccurrence(1);
		if ( !"Termination".equals(terminationOcc.getName()) ){
			throw new RuntimeException("This is not the Termination occurrence, it is "+terminationOcc.getName());
		}
        ...
        for ( Record record: records ){
            ...
            record.attach(ds);
            Map<Long, List<Object[]>> recData = new HashMap<Long, List<Object[]>>();
            data.put(record.getId(), recData);
            preApplyPatchForOccurrence(recData, scid, transitionOcc, record, ds, client, saml);
            preApplyPatchForOccurrence(recData, scid, terminationOcc, record, ds, client, saml);
        }
	...
	private void preApplyPatchForOccurrence(Map<Long, List<Object[]>> recData,
											Document scid,
											DocumentOccurrence scidOcc,
											Record record,
											DataSet ds,
        ...
        if ( null != scidInst ){
        	...
        	List<Object[]> occData = new ArrayList<Object[]>();
        	recData.put(scidOcc.getId(), occData);
            ...
            System.out.println("Saving record "+record.getIdentifier().getIdentifier()+" for occurrence "+scidOcc.getName());
	...
	public void postApplyPatch(DataSet ds, Object obj, RepositoryClient client, String saml) throws Exception {
		...
		//    1st is the additional "other" text for the option (if applicable)
		//    2nd is the change comment (if applicable)
		Map<Long, Map<Long, List<Object[]>>> data = (Map<Long, Map<Long, List<Object[]>>>)obj;
        ...
        for ( Record record: records ){
            System.out.println("Processing record "+record.getIdentifier().getIdentifier());
            record.attach(ds);
            Map<Long, List<Object[]>> recData = data.get(record.getId());
            if ( null != recData ){
            	postApplyPatchForOccurrence(recData, scid, transitionOcc, record, ds, client, saml, stdCodes);
            	postApplyPatchForOccurrence(recData, scid, terminationOcc, record, ds, client, saml, stdCodes);
            }
        }
	...
	private void postApplyPatchForOccurrence(Map<Long, List<Object[]>> recData,
											Document scid,
											DocumentOccurrence scidOcc,
											Record record,
											DataSet ds,
											RepositoryClient client,
											String saml,
											List<StandardCode> stdCodes) throws Exception {
        ...
        if ( null != scidInst ){
            ...
            List<Object[]> scidVals = recData.get(scidOcc.getId());
