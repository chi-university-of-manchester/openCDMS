...
import org.psygrid.edie.ED2Groups;
 ...
 *
 */
public class InstallEDIEControlPolicy {

	public static void main(String[] args){

		GroupType[] groups = ED2Groups.allGroups();

		PolicyDescriptionType pdt = new PolicyDescriptionType();
		pdt.setActions(RBACAction.allActions());
		TargetType[] tta = new TargetType[ED2Groups.allAsTargets().length+RBACTarget.allAsTargets().length];
		for (int i = 0; i < ED2Groups.allAsTargets().length; i++) {
			tta[i] = ED2Groups.allAsTargets()[i];
		}
		int j=0;
		for (int i = ED2Groups.allAsTargets().length; i < ED2Groups
				.allAsTargets().length
		...
		PrivilegeType[] prta = new PrivilegeType[ED2Groups.allAsPrivileges().length
				+ RBACRole.allAsPrivileges().length];
		for (int i = 0; i < ED2Groups.allAsPrivileges().length; i++) {
			prta[i] = ED2Groups.allAsPrivileges()[i];
		}
		int k=0;
		for (int i = ED2Groups.allAsPrivileges().length; i < ED2Groups
		...
		try {
			...
			try {
				...
				PolicyType pt = new PolicyType("EDIE Control", "EDC",pdt, sta);
