 ...
 *
 */
public class InstallCommandPolicy {

	public static void main(String[] args){

		GroupType[] groups = COMGroups.allGroups();

		PolicyDescriptionType pdt = new PolicyDescriptionType();
		pdt.setActions(RBACAction.allActions());
		TargetType[] tta = new TargetType[COMGroups.allAsTargets().length+RBACTarget.allAsTargets().length];
		for (int i = 0; i < COMGroups.allAsTargets().length; i++) {
			tta[i] = COMGroups.allAsTargets()[i];
		}
		int j=0;
		for (int i = COMGroups.allAsTargets().length; i < COMGroups
				.allAsTargets().length
		...
		PrivilegeType[] prta = new PrivilegeType[COMGroups.allAsPrivileges().length
				+ RBACRole.allAsPrivileges().length];
		for (int i = 0; i < COMGroups.allAsPrivileges().length; i++) {
			prta[i] = COMGroups.allAsPrivileges()[i];
		}
		int k=0;
		for (int i = COMGroups.allAsPrivileges().length; i < COMGroups
		...
		try {
			...
			try {
				...
				PolicyType pt = new PolicyType("Command", "COM",pdt, sta);
