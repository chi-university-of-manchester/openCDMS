 ...
 *
 */
public class InstallCommandTestPolicy {

	public static void main(String[] args){

		GroupType[] groups = CMTGroups.allGroups();

		PolicyDescriptionType pdt = new PolicyDescriptionType();
		pdt.setActions(RBACAction.allActions());
		TargetType[] tta = new TargetType[CMTGroups.allAsTargets().length+RBACTarget.allAsTargets().length];
		for (int i = 0; i < CMTGroups.allAsTargets().length; i++) {
			tta[i] = CMTGroups.allAsTargets()[i];
		}
		int j=0;
		for (int i = CMTGroups.allAsTargets().length; i < CMTGroups
				.allAsTargets().length
		...
		PrivilegeType[] prta = new PrivilegeType[CMTGroups.allAsPrivileges().length
				+ RBACRole.allAsPrivileges().length];
		for (int i = 0; i < CMTGroups.allAsPrivileges().length; i++) {
			prta[i] = CMTGroups.allAsPrivileges()[i];
		}
		int k=0;
		for (int i = CMTGroups.allAsPrivileges().length; i < CMTGroups
			...
			*/
			...
			try {
				...
				StatementType st = new StatementType(RBACTarget.ANY.toTargetType(),
						RBACAction.ANY.toActionType(), rule);
				StatementType[] sta = new StatementType[] { st };
				PolicyType pt = new PolicyType("COMMAND TEST", "CMT", pdt, sta);
				PolicyType[] pta = new PolicyType[] { pt };
