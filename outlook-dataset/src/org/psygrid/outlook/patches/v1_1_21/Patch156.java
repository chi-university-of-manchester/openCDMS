...
package org.psygrid.outlook.patches.v1_1_21;
...
public class Patch156 extends AbstractPatch {
	...
	public void applyPatch(DataSet ds, String saml) throws Exception {
		...
		for (IReport report: reports) {
			if (report instanceof IManagementReport) {
				if ("Outlook - Record Status Report".equals(report.getTitle())) {
					System.out.println("Updating Outlook - Record Status Report");
					((IManagementReport)report).setViewAction(RBACAction.ACTION_DR_STATUS_REPORT);
				}
				else if ("Outlook - Document Status Report".equals(report.getTitle())) {
					System.out.println("Updating Outlook - Document Status Report");
					((IManagementReport)report).setViewAction(RBACAction.ACTION_DR_STATUS_REPORT);
				}
				else {
					System.out.println("Updating "+report.getTitle());
