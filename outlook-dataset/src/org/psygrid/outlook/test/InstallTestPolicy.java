 ...
 *
 */
public class InstallTestPolicy {

	public static void main(String[] args){

		GroupType[] groups = TSTGroups.allGroups();

		PolicyDescriptionType pdt = new PolicyDescriptionType();
		pdt.setActions(RBACAction.allActions());

		TargetType[] tta = new TargetType[TSTGroups.allAsTargets().length+RBACTarget.allAsTargets().length];
		for (int i = 0; i < TSTGroups.allAsTargets().length; i++) {
			tta[i] = TSTGroups.allAsTargets()[i];
		}
		int j=0;
		for (int i = TSTGroups.allAsTargets().length; i < TSTGroups
				.allAsTargets().length
		...
		PrivilegeType[] prta = new PrivilegeType[TSTGroups.allAsPrivileges().length
				+ RBACRole.allAsPrivileges().length];
		for (int i = 0; i < TSTGroups.allAsPrivileges().length; i++) {
			prta[i] = TSTGroups.allAsPrivileges()[i];
		}
		int k=0;
		for (int i = TSTGroups.allAsPrivileges().length; i < TSTGroups
		...
		try {
			...
			try {
				...
				PolicyType pt = new PolicyType("Test", "TST", pdt, sta);
