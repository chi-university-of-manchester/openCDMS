...
package org.psygrid.drn.address.patches.v1_1_24;
	...
	public void applyPatch(DataSet ds, String saml) throws Exception {
		...
		for (IReport report: reports) {
			if (report instanceof IManagementReport) {
				if ("ADDRESS - Record Status Report".equals(report.getTitle())) {
					System.out.println("ADDRESS - Record Status Report");
					((IManagementReport)report).setViewAction(RBACAction.ACTION_DR_STATUS_REPORT);
				}
				else if ("ADDRESS - Document Status Report".equals(report.getTitle())) {
					System.out.println("Updating ADDRESS - Document Status Report");
					((IManagementReport)report).setViewAction(RBACAction.ACTION_DR_STATUS_REPORT);
				}
				else {
