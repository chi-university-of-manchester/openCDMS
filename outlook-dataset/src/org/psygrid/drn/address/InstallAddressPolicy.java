...
public class InstallAddressPolicy {
	...
	public static void main(String[] args){
		...
		TargetType[] tta = new TargetType[ADDGroups.allAsTargets().length+RBACTarget.allAsTargets().length];
		for (int i = 0; i < ADDGroups.allAsTargets().length; i++) {
			tta[i] = ADDGroups.allAsTargets()[i];
		}
		int j=0;
		for (int i = ADDGroups.allAsTargets().length; i < ADDGroups
				.allAsTargets().length
		...
		PrivilegeType[] prta = new PrivilegeType[ADDGroups.allAsPrivileges().length
				+ RBACRole.allAsPrivileges().length];
		for (int i = 0; i < ADDGroups.allAsPrivileges().length; i++) {
			prta[i] = ADDGroups.allAsPrivileges()[i];
		}
		int k=0;
		for (int i = ADDGroups.allAsPrivileges().length; i < ADDGroups
