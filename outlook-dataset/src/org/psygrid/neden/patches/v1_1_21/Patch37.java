	...
	public void applyPatch(DataSet ds, String saml) throws Exception {
		...
		for (IReport report: reports) {
			if (report instanceof IManagementReport) {
				if ("National Eden - Record Status Report".equals(report.getTitle())) {
					System.out.println("National Eden - Record Status Report");
					((IManagementReport)report).setViewAction(RBACAction.ACTION_DR_STATUS_REPORT);
				}
				else if ("National Eden - Document Status Report".equals(report.getTitle())) {
					System.out.println("Updating National Eden - Document Status Report");
					((IManagementReport)report).setViewAction(RBACAction.ACTION_DR_STATUS_REPORT);
				}
				else {
