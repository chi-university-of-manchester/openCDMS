...
package org.psygrid.neden.patches.future;
...
public class Patch49 extends AbstractPatch {
	...
	public void applyPatch(DataSet ds, String saml) throws Exception {
		for (Document doc: ((DataSet)ds).getDocuments()) {
			if (doc.getDisplayText().equals("Personal Details Form")) {
	...
	private void updatePersonalDetailsForm(Document doc) {
		...
		for (Entry entry: doc.getEntries()) {
			if ("Had previous antipsychotic Medication".equals(entry.getName())) {
				System.out.println("Had previous antipsychotic Medication");
				counter++;
				...
				for (Entry e: ce.getEntries()) {
					if ("Drug Option".equals(e.getName())) {
						System.out.println("Drug Option");
						counter++;
