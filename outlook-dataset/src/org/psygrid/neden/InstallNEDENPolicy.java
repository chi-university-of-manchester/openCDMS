 ...
 *
 */
public class InstallNEDENPolicy {

	public static void main(String[] args) {

		GroupType[] groups = NEDGroups.allGroups();

		PolicyDescriptionType pdt = new PolicyDescriptionType();
		pdt.setActions(RBACAction.allActions());
		TargetType[] tta = new TargetType[NEDGroups.allAsTargets().length+RBACTarget.allAsTargets().length];
		for (int i = 0; i < NEDGroups.allAsTargets().length; i++) {
			tta[i] = NEDGroups.allAsTargets()[i];
		}
		int j=0;
		for (int i = NEDGroups.allAsTargets().length; i < NEDGroups
				.allAsTargets().length
		...
		PrivilegeType[] prta = new PrivilegeType[NEDGroups.allAsPrivileges().length
				+ RBACRole.allAsPrivileges().length];
		for (int i = 0; i < NEDGroups.allAsPrivileges().length; i++) {
			prta[i] = NEDGroups.allAsPrivileges()[i];
		}
		int k=0;
		for (int i = NEDGroups.allAsPrivileges().length; i < NEDGroups
		...
		try {
			...
			try {
				...
				PolicyType pt = new PolicyType("National EDEN", "NED", pdt, sta);
