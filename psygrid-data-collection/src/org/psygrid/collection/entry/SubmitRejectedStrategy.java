...
public class SubmitRejectedStrategy extends SubmitDocumentStrategy {

    public SubmitRejectedStrategy(Application application) {
        super(application);
    }
    ...
    public void submit(final Record currentRecord) {
      ...
      //  IChangeHistory change = docInstance.addToHistory(SecurityManager.getInstance().getUserName());
        ...
        SwingWorker<Object, Object> worker = new SwingWorker<Object, Object>(){
            ...
            protected Object doInBackground() throws ConnectException, SocketTimeoutException, 
                ...
                if ( null != docInstance.getRecord().getSecondaryRecord() ){
                    ...
                    DocumentInstance secondaryDocInstance = DdeHelper.findDocInstForSecondary(docInstance, secondaryRecord);
                    secondaryDocInstance.addToHistory(SecurityManager.getInstance().getUserName());
                    RemoteManager.getInstance().saveRecord(secondaryRecord);
                    RemoteManager.getInstance().changeDocumentInstanceStatus(secondaryDocInstance,
                            DocumentStatus.PENDING);
                    PersistenceManager.getInstance().changeLocalDocInstanceStatus(secondaryDocInstance, DocumentStatus.PENDING);
                }
