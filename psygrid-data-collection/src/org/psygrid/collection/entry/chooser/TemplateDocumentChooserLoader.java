 ...
 *
 */
public class TemplateDocumentChooserLoader extends SwingWorker<ChoosableList, Object> {
    private static final Log LOG = LogFactory.getLog(TemplateDocumentChooserLoader.class);

    private Application application;

    public TemplateDocumentChooserLoader(Application application) {
    ...
    protected ChoosableList doInBackground()    {
        ...
        synchronized (PersistenceManager.getInstance()) {
        	List<DataSetSummary> dssList = null;
        	if ( RemoteManager.getInstance().isTestDataset() ){
        		...
        		try{
        			...
        			dssList = new ArrayList<DataSetSummary>();
        			dssList.add(dss);
        		}
    ...
    protected void done() {
        ...
        new TemplateDocumentChooserDialog(application, choosableList, null);
